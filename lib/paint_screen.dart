...
import 'package:mutliplayer_game/final_leaderboard.dart';
import 'package:mutliplayer_game/home_screen.dart';
import 'package:mutliplayer_game/models/my_custom_painter.dart';
import 'package:mutliplayer_game/models/touch_points.dart';
import 'package:mutliplayer_game/sidebar/player_scoreboard_drawer.dart';
import 'package:mutliplayer_game/wating_lobby_screen.dart';
...
class PaintScreen extends StatefulWidget {
  ...
  //so this data we reciever from there has to pass to node.js backend
...
class _PaintScreenState extends State<PaintScreen> {
  ...
  void startTimer() {
    const onseSec = const Duration(seconds: 1);
    _timer = Timer.periodic(onseSec, (Timer time) {
      if (_start == 0) {
  ...
  void connect() {
    _socket = IO.io('http://192.168.106.162:3000', <String, dynamic>{
      'transports': ['websocket'],
      'autoConnect': false
    });
    _socket.connect();

    //if screen is coming from createroom then we pass it to socket and socket pass it to node js
    if (widget.screenFrom == 'createRoom') {
      //the other is the data we want to pass to the node hs
